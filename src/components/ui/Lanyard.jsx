...
function Band({
  ...
  const { nodes, materials } = useGLTF(cardGLB || '/assets/3d/card.glb');
  const texture = useTexture(lanyardImage || lanyard || '/assets/3d/lanyard.png');
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img, rect) => {
      ...
      const scale = pick(rw / (img.naturalWidth || img.width || 1), rh / (img.naturalHeight || img.height || 1));
      const dw = (img.naturalWidth || img.width || 1) * scale;
      const dh = (img.naturalHeight || img.height || 1) * scale;
      const dx = rx + (rw - dw) / 2;
      const dy = ry + (rh - dh) / 2;
      ctx.save();
      ctx.beginPath();
