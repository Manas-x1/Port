...
const DEFAULT_SRC = '/Potrait1.jpeg';
...
vec2 coverUv(vec2 uv) {
  ...
  return (uv - 0.5) * s + 0.5 + uFocalOffset;
}
...
const HalftoneReveal = ({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uFocalOffset: { value: focalOffset },
    ...
    const onImgLoad = () => {
      texture.image = img;
      uniforms.uImageSize.value = [img.naturalWidth || 1, img.naturalHeight || 1];
    };
    if (img.complete) {
      onImgLoad();
    } else {
  ...
  useEffect(() => {
    ...
    u.uFocalOffset.value = focalOffset;
  }, [
    dotSize,
    dotDensity,
    angle,
